...
class PplCvOclIntegralTest : public ::testing::TestWithParam<Parameters> {
 public:
  PplCvOclIntegralTest() {
...
template <typename Tsrc, typename Tdst, int channels>
bool PplCvOclIntegralTest<Tsrc, Tdst, channels>::apply() {
  assert(channels == 1);
  cv::Mat src;
  ...
  int dst_size = (size.height + 1) * (size.width + 1) * sizeof(Tdst);
  cl_int error_code = 0;
  ...
  error_code = clEnqueueWriteBuffer(queue, gpu_src, CL_FALSE, 0, src_size,
  ...
  // ppl::cv::ocl::Integral<Tsrc, Tdst, 1>(queue, 
  //     size.height, size.width, size.width * sizeof(Tsrc), gpu_src, 
  //     size.height + 1, size.width + 1, (size.width + 1) * sizeof(Tdst), gpu_dst);

  // ppl::cv::ocl::Integral<Tsrc, Tdst, 1>(queue, 
  //     size.height, size.width, size.width * sizeof(Tsrc), gpu_input, 
  //     size.height + 1, size.width + 1, (size.width + 1) * sizeof(Tdst), gpu_output);

  ppl::cv::ocl::Integral<Tsrc, Tdst, 1>(queue, 
      size.height, size.width, size.width, gpu_src, 
      size.height + 1, size.width + 1, (size.width + 1), gpu_dst);

  ppl::cv::ocl::Integral<Tsrc, Tdst, 1>(queue, 
      size.height, size.width, size.width, gpu_input, 
      size.height + 1, size.width + 1, (size.width + 1), gpu_output);

  error_code = clEnqueueReadBuffer(queue, gpu_dst, CL_TRUE, 0, dst_size,
  ...
  bool identity0 = checkMatricesIdentity<Tdst>((const Tdst*)cv_dst.data, 
      cv_dst.rows, cv_dst.cols, cv_dst.channels(), cv_dst.cols * cv_dst.channels() * sizeof(Tdst), 
      (const Tdst*)dst.data, (size.width + 1) * sizeof(Tdst), epsilon);
  bool identity1 = checkMatricesIdentity<Tdst>((const Tdst*)cv_dst.data, 
      cv_dst.rows, cv_dst.cols, cv_dst.channels(), cv_dst.cols * cv_dst.channels() * sizeof(Tdst), 
      output, (size.width + 1) * sizeof(Tdst), epsilon);

  error_code = clEnqueueUnmapMemObject(queue, gpu_output, output, 0, NULL, NULL);
  CHECK_ERROR(error_code, clEnqueueUnmapMemObject);
...
using PplCvOclIntegralTest ## Tsrc ## Tdst ## channels =                       \
        PplCvOclIntegralTest<Tsrc, Tdst, channels>;                            \
TEST_P(PplCvOclIntegralTest ## Tsrc ## Tdst ## channels, Standard) {           \
...
INSTANTIATE_TEST_CASE_P(IsEqual,                                               \
  ...
  [](const testing::TestParamInfo<                                             \
      PplCvOclIntegralTest ## Tsrc ## Tdst ## channels::ParamType>& info) {    \
